...
/**
 * An example app for KMeans
 * Run with
 * {{{
 * bin/run-example ml.KMeansExample [options] <input>
 * }}}
 */
object KMeansExample {
  ...
  case class Params(
      ...
      initSteps: Int = 5,
      dataFormat: String = "tsv") extends AbstractParams[Params]
  ...
  private def run(params: Params): Unit = {
    ...
    val dataset = loadData(sc, params.input, params.dataFormat)
  ...
  private def parseArgs(args: Array[String]): Option[Params] = {
    ...
    val parser = new OptionParser[Params]("KMeansExample") {
      ...
      opt[String]("dataFormat")
        .text(s"data format (tsv or csv), default: ${defaults.dataFormat}")
  ...
  private def loadData(sc: SparkContext, input: String, dataFormat: String): DataFrame = {
    val sqlContext = new SQLContext(sc)
    import org.apache.spark.mllib.linalg.VectorUDT

    val sep = dataFormat match {
      case "tsv" => """\t"""
      case "csv" => ""","""
      case _ => throw new IllegalArgumentException(s"dataFormat option should be tsv or csv")
    }

    val rowRDD = sc.textFile(input).filter(l => l != "")
      .map(_.split(sep).map(v => java.lang.Double.parseDouble(v)))

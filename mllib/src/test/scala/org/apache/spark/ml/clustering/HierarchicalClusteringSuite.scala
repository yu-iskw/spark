...
object HierarchicalClusteringSuite {
  ...
  def generateTestDataFrameWithSparseVectors(
...
class HierarchicalClusteringSuite extends SparkFunSuite with MLlibTestSparkContext with Logging {
  ...
  test("fit, predict and transform with sparse vectors") {
    ...
    val algo = new HierarchicalClustering()
        ...
        .setSeed(1)
    val dataset =
      HierarchicalClusteringSuite.generateTestDataFrameWithSparseVectors(sqlContext, 5, 99)
    val model = algo.fit(dataset)

...
import breeze.linalg.{DenseVector => BDV, mpow, norm}
...
/**
  ...
  */
class AdaGrad private[spark] (private var gradient: Gradient, private var updater: Updater)
  ...
  def optimize(data: RDD[(Double, Vector)], initialWeights: Vector): Vector = {
    val (weights, _) = AdaGrad.runMiniBatchSGD(
      data,
      gradient,
      updater,
      stepSize,
...
/**
  ...
  */
@DeveloperApi
object AdaGrad extends Logging {
  /**
  ...
  def runMiniBatchSGD(
    ...
    while (!converged && i <= numIterations) {
      ...
      val (gradientSum, gradientSquaredSum, lossSum, miniBatchSize) =
  ...
  def runMiniBatchSGD(
    ...
    AdaGrad.runMiniBatchSGD(data, gradient, updater, stepSize, numIterations,
